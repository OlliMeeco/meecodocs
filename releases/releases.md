...
* Hid the `owner_id` field from share objects when the sharee is not connected with the owner of the item (e.g. on-share)
...
* To enforce data integrity, once a slot has an attachment, it is not permitted to remove or replace the attachment
...
* `PUT /slots/:id` removed. Instead, `PUT /items/:id` should be used
* Added attachment folders functionality
  Managing attachments folders not linked to any slots:
